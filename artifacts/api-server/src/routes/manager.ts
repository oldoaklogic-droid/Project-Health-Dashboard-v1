...
function projectSummary(project: Awaited<ReturnType<typeof computePortfolio>>["projects"][number]) {
  return {
    ...
    pm: project.pm,
    fee: project.fee,
    noContractAmountOnFile: project.noContractAmountOnFile,
    closeoutCandidate: project.closeoutCandidate,
...
router.get("/manager/portfolio", async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    scoreboard: {
      ...
      noContractAmountOnFileCount: rows.filter((row) => row.noContractAmountOnFile).length,
      closeoutCandidateCount: rows.filter((row) => row.closeoutCandidate).length,
      redCount: bySeverity("red").length,
      yellowCount: bySeverity("yellow").length,
      grayCount: bySeverity("gray").length,
    },
    snapshot: portfolio.snapshot,
    vsAug30: portfolio.comparison,
    excludedInternalClients: portfolio.excludedInternalClients,
    red: bySeverity("red"),
